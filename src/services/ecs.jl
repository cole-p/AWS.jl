...
For services using the rolling update (ECS) you can update the desired count, the
deployment configuration, the network configuration, load balancers, service registries,
enable ECS managed tags option, propagate tags option, task placement constraints and
strategies, and the task definition. When you update any of these parameters, Amazon ECS
starts new tasks with the new configuration.  For services using the blue/green
(CODE_DEPLOY) deployment controller, only the desired count, deployment configuration, task
...
- `"loadBalancers"`: A list of Elastic Load Balancing load balancer objects. It contains
  ...
  tasks are running. You can remove existing loadBalancers by passing an empty list.
